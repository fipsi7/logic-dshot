...
DshotAnalyzerResults::DshotAnalyzerResults(DshotAnalyzer* analyzer,
                                           DshotAnalyzerSettings* settings)
    : AnalyzerResults(), mSettings(settings), mAnalyzer(analyzer) {}

DshotAnalyzerResults::~DshotAnalyzerResults() {}

void DshotAnalyzerResults::GenerateBubbleText(U64 frame_index, Channel& channel,
                                              DisplayBase display_base) {
  ClearResultStrings();
  Frame frame = GetFrame(frame_index);

  char number_str[128];
  AnalyzerHelpers::GetNumberString(frame.mData1, display_base, 11, number_str,
                                   128);
...
void DshotAnalyzerResults::GenerateExportFile(const char* file,
                                              DisplayBase display_base,
...
void DshotAnalyzerResults::GenerateFrameTabularText(U64 frame_index,
                                                    DisplayBase display_base) {
...
void DshotAnalyzerResults::GeneratePacketTabularText(U64 packet_id,
                                                     DisplayBase display_base) {
  // ClearTabularText();
  // AddTabularText( "not supported" );
}

void DshotAnalyzerResults::GenerateTransactionTabularText(
