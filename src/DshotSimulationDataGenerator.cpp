...
void DshotSimulationDataGenerator::Initialize(U32 simulation_sample_rate,
...
U32 DshotSimulationDataGenerator::GenerateSimulationData(
...
void DshotSimulationDataGenerator::CreateChannelUpdate() {
  uint32_t samples_per_bit =
      mSimulationSampleRateHz / (mSettings->mDshotRate * 1000);

  uint16_t channel = 0;

  uint16_t chan_val =
      (2047.0 / 2.0) + (2047.0 / 2.0) * std::sin(mChannelRadians);
  bool telem = chan_val > 2045;
  chan_val <<= 5;
  uint8_t crc = ((chan_val >> 4) & 0xf) ^ ((chan_val >> 8) & 0xf) ^
                ((chan_val >> 12) & 0xf);
  ...
  for (int i = sizeof(channel) * 8 - 1; i >= 0; i--) {
    ...
    mSerialSimulationData.Advance(static_cast<double>(mSimulationSampleRateHz) *
                                  (high * 1e-9));
    mSerialSimulationData.TransitionIfNeeded(BIT_LOW);
    mSerialSimulationData.Advance(static_cast<double>(mSimulationSampleRateHz) *
                                  (low * 1e-9));
